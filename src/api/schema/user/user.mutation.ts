...
export const userMutation = extendType({
    ...
    definition(t) {
        t.field("createUserAdmin", {
            ...
            resolve: async (_, { Auth: { email }, Profile: { lastname, firstname, birthday, phone }, companyName }): Promise<any> => {
        ...
        t.field("createUserAccount", {
            ...
            resolve: async (_, { email, Profile: { birthday, firstname, lastname, phone }, role, companyName }, { req }): Promise<any> => {
                const pass = await bcrypt.hash(new Date(birthday).toISOString().slice(0, 10).replaceAll("-", ""), 12);


                const token = req.cookies[ 'company_access_token' ]

                const { userId }: any = await verify(token, "compayName")
        ...
        t.field("login", {
            ...
            resolve: async (_, { email, password }, { res }): Promise<any> => {
                ...
                return await prisma.$transaction(async () => {
                    ...
                    if (user.locked) throw new GraphQLError("You account has been locked.")

                    if (!user) throw new GraphQLError("No existing email address");
                    const pass = await bcrypt.compare(password, user.password)

                    if (!pass) throw new GraphQLError("Invalid password. Please Try again");
        ...
        t.field("userlockedAccount", {
            ...
            resolve: async (_, { userID, locked }): Promise<any> => {
        ...
        t.field("updateUserProfile", {
            ...
            resolve: async (_, { userID, profile: { firstname, birthday, lastname, phone } }): Promise<any> => {
